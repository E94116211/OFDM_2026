...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \\{\scriptsize
    \[x(t)=\begin{cases}
    X_{1}^{prev}e^{j2\pi f_{1}t}\ +\ X_{2}^{prev}e^{j2\pi f_{2}t},&0.4\le t<0.5\\
    X_{1}^{curr}e^{j2\pi f_{1}(t-0.5)}\ +\ X_{2}^{curr}e^{j2\pi f_{2}(t-0.5)},&0.5\le t<0.9
    \end{cases}\]}
    \item Number of sampling points=N, $T_s=\frac{0.5}{N}s$, $t_s=0.4+nT_s$, $f_{1}=2$Hz, $f_{2}=4$Hz
    \\{\scriptsize
    \[x[n]=\begin{cases}
    X_{1}^{prev}e^{j2\pi f_{1}(0.4+nT_{s})}\ +\ X_{2}^{prev}e^{j2\pi f_{2}(0.4+nT_{s})},&0\le n<0.2N\\
    X_{1}^{curr}e^{j2\pi f_{1}(-0.1+nT_{s})}\ +\ X_{2}^{curr}e^{j2\pi f_{2}(-0.1+nT_{s})},&0.2N\le n<N
    \end{cases}\]}
  \end{itemize}
\end{itemize}
\end{frame}
...
\begin{itemize}
  ...
  ={}&
  \sum_{m=1}^{2}
  X_m^{prev}\alpha_{m\rightarrow k}
  +
  \sum_{m=1}^{2}
  X_m^{curr}\beta_{m\rightarrow k}.
  \end{aligned}
  \]
  }
\end{itemize}
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \\{\scriptsize
    \[x(t)=\begin{cases}
    X_{1}^{prev}e^{j2\pi f_{1}(t+0.1)}\ +\ X_{2}^{prev}e^{j2\pi f_{2}(t+0.1)},&-0.1\le t<0\\
    X_{1}^{curr}e^{j2\pi f_{1}t}\ +\ X_{2}^{curr}e^{j2\pi f_{2}t},&0\le t<0.4
    \end{cases}\]}
    \item Number of sampling points=N, $T_s=\frac{0.5}{N}s$, $f_{1}=2$Hz, $f_{2}=4$Hz
    \\{\scriptsize
    \[x[n]=\begin{cases}
    X_{1}^{prev}e^{j2\pi f_{1}(nT_{s}+0.1)}\ +\ X_{2}^{prev}e^{j2\pi f_{2}(nT_{s}+0.1)},&-0.2N\le n<0\\
    X_{1}^{curr}e^{j2\pi f_{1}nT_{s}}\ +\ X_{2}^{curr}e^{j2\pi f_{2}nT_{s}},&0\le n<0.8N
    \end{cases}\]}
  \end{itemize}
\end{itemize}
\end{frame}
...
\begin{itemize}
  ...
  \begin{flalign*}
    &Y[k]&
    \\=&\frac{1}{N}\sum_{n=-0.2N}^{-1}\sum_{m=1}^{2}(X_{m}^{prev}e^{j2\pi f_{m}0.1 e^{j2\pi \frac{m}{N}n}})e^{-j2\pi \frac{k}{N}n}&
    \\+&\frac{1}{N}\sum_{n=0}^{0.8N-1}\sum_{m=1}^{2}(X_{m}^{curr}e^{j2\pi \frac{m}{N}n})e^{-j2\pi \frac{k}{N}n}&
    \\=&\sum_{m=1}^{2}X_{m}^{prev}\cdot \alpha_{m\rightarrow k}+\sum_{m=1}^{2}X_{m}^{curr}\cdot \beta_{m\rightarrow k}&
  \end{flalign*}
\end{itemize}
\end{frame}
%-----------------------------------------------------------------------------%
...
\begin{itemize}
  \item When $m=k$,
  \begin{flalign*}
    &\alpha_{k\rightarrow k} =\frac{e^{j0.2\pi f_{k}}}{N}(0.2N)=0.2\cdot e^{j0.2\pi f_{k}} \not= 0\ \ \text{(ISI)} &\\
...
\begin{itemize}
  \item When $m\not= k$, 
  \begin{flalign*}
    &\alpha_{m\rightarrow k}=\frac{e^{j0.2\pi f_{m}}}{N}\frac{e^{\pm j2\pi (-0.2)-1}}{1-e^{\pm j2\pi \frac{1}{N}}}\not= 0\ \ \text{(ISI, ICI)}&\\
...
\begin{itemize}
    ...
    \[\begin{cases}
    Y[1]=0.8X_{1}^{curr}+\beta_{2\rightarrow 1}X_{2}^{curr}+\alpha_{2\rightarrow 1}X_{2}^{prev}\\
    Y[2]=0.8X_{2}^{curr}+\beta_{1\rightarrow 2}X_{1}^{curr}+\alpha_{1\rightarrow 2}X_{1}^{prev}
    \end{cases}\]
