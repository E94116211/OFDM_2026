...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \end{itemize}
        \item Sampling Clock Offset (SCO):
        \begin{itemize}
            \item ADC/DAC Clock Mismatch
            \item The offset is proportional to the subcarrier index.
        \end{itemize}
        \item Phase Noise:
        \begin{itemize}
            \item Local Oscillator Short-term Instability
            \item Construct a zero-mean Wiener random process to model the phase noise.
        \end{itemize}
    \end{itemize}
\end{frame}
%-----------------------------------------------------------------------------%
...
\begin{itemize}
  ...
  \begin{flalign*}
    ...
    \\+&\frac{1}{N}\sum_{n=0}^{0.8N-1}\sum_{m=1}^{2}(X_{m}^{curr}e^{j2\pi \frac{m}{N}})e^{-j2\pi \frac{k}{N}n}&
...
\begin{itemize}
    ...
    \[\begin{cases}
    Y[1]=0.8X_{1}^{curr}+\beta_{2\rightarrow 1}X_{2}^{curr}\\
    Y[2]=0.8X_{2}^{curr}+\beta_{1\rightarrow 2}X_{1}^{curr}
    \end{cases}\]
